...
function renderHome() {
  ...
  // 身边的人
  out.push(`<h4>枕边</h4><div class="card">`);
  if (P) {
    ...
    const cand = S.npcs.filter(n => n.rel >= 55 && !/爱人|前任|家里人|妈|爸/.test(n.tie || ''));
    out.push(`<div class="tip">眼下一个人过。</div>
      ${cand.length ? `<div class="cardhd" style="margin-top:8px">能开口的人</div>` + cand.map(n =>
        `<div class="li"><b>${esc(n.name)}</b><span class="rel">${relWord(n.rel, n.tie)}</span>
          <div class="tip">${esc(n.job || '')}${n.tie ? '｜' + esc(n.tie) : ''}
            <button class="ghost sm" onclick="askLove('${esc(n.name)}')">挑明</button></div></div>`).join('')
        : '<div class="tip">还没有走得够近的人——关系到「朋友」以上才谈得上这个。</div>'}`);
  }
  out.push(`</div>`);
