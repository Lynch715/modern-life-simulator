...
- 跟下面的字数、结尾写法、对话条数、选项要求有冲突时，一律以这里为准；篇幅不够就加长，写到一千字以上也可以。
- 括号里的话本身不要原样抄进叙事。
`;
...
function segPrompt(seg) {
  ...
  const actBlock = S.lastAction
    ? `【主角自己安排的事】${S.lastAction}\n（这一段先把这件事的经过写足——至少 200 字，照玩家写的去办：他写了几件事就写几件，写了去哪、找谁、带什么、怎么说，一样不落，写过程而不是一句话交代结果——然后再往下走）`
    : '（这是开局之后的第一段）';
  return `${worldRules()}
...
要求：
${E.fdm(S).fiat && S.lastAction ? `- **这一段的头等大事**：把玩家写的「${S.lastAction}」写成已经办成的事，写足、写具体、写出后续的好处。这一条压过下面所有要求。\n` : ''}- 叙事 ${fiatAsks().length ? '按括号里的要求写足，至少 600 字，不设上限' : days >= 10 ? '400-600' : '250-420'} 字。${days >= 8 ? '这是一段被快进的日子，不许写成"第一天……第二天……"的流水账。挑这段时间里真正有分量的两三件事写，其余用一两句带过。' : ''}
...
要求：
${E.fdm(S).fiat ? `- **这一段的头等大事**：玩家写的「${S.lastAction}」已经成了，你只负责写它怎么成的，写足、写出后续的好处。这一条压过下面所有要求。\n` : ''}- **只写这一两天，就写他去做「${fiatAsks().length ? splitAct(S.lastAction).doing : S.lastAction}」这件事**。${fiatAsks().length ? '篇幅按括号里的要求来，至少 600 字，不设上限。' : '450-700 字。'}
